...
class SpriteObject implements IGameObject, IParentChild {
    ...
    createNew(x: number, y: number, textureName: string, frame: string | null = null): SpriteObject {
        let sprite = new SpriteObject(this._entity.createNew(), this._childHandler.createNew(), this._screen, this._input.createNew());
